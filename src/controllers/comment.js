...
export const list = async (req, res) => {
    try {
        const comment = await Comment.find().populate('User').populate('Product').sort({ "createdAt": -1 })
        res.json(comment)
    } catch (error) {
        res.status(400).json(
...
export const getOne = async (req, res) => {
    try {
        const comment = await Comment.findOne({ _id: req.params.id }).populate('User').populate('Product')
        res.json(comment)
    } catch (error) {
        res.status(400).json(
